...
<template>
    <div v-if="data.length > 0" class="grid grid-cols-2 gap-4">
        <router-link v-for="komik in data" :key="komik" :to="`/komik/${komik.href}`" class="relative w-full h-80 rounded-lg">
...
export default {
    ...
    mounted() {
        fetch(`https://mite-mail.my.id/komikcast/genres/${this.$route.params.genre}/page/${this.page}`, {
            method: 'GET',
            headers: {
        ...
        window.addEventListener('scroll', () => {
            ...
            if (window.innerHeight + window.scrollY >= document.body.offsetHeight) {
                if (this.$route.path !== `/genres/${this.$route.params.genre}`) return
                this.loading = true
                fetch(`https://mite-mail.my.id/komikcast/genres/${this.$route.params.genre}/page/${this.page}`)
                    .then(res => res.json())
                    .then(res => {
